defmodule ExTwitterStreamTest do
  ...
  defp wait_async_request_initialization do
    # Put small wait for mocking library to become ready.
    :timer.sleep(100)
  end
  ...
  defp start_stream({pid, request_id}) do
    headers = [
      {'connection', 'close'},
      {'date', 'Sun, 06 Jul 2014 14:48:13 UTC'},
  ...
  defp send_part({pid, request_id}, json), do: send(pid, {:http, {request_id, :stream, json}})
